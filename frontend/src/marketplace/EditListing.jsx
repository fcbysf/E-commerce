...
export default function EditListing(){
  ...
  const removeImage = (image, index) => {
    ...
    if(image.image && !deletedImgsIds.includes(image.id)){
      setDeletedImgsIds([...deletedImgsIds, image.id])
    }
  ...
  const handleSubmit = (e) => {
    ...
    if(deletedImgsIds){
      deletedImgsIds.forEach(id=>{
        formDataSubmit.append("deletedImgsIds[]", id);
      })
    }
                    ...
                    {images.length > 0 ? (
                      <>
                        <div className="aspect-square flex items-center justify-center p-4">
                          <img
                            src={images[previewIndex].preview|| images[previewIndex].image}
                            alt="Preview"
